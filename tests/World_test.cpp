...
BOOST_AUTO_TEST_CASE(new_particles)
{
    ...
    BOOST_CHECK(!i.check_overlap(p1.second.as_sphere(), array_gen(p1.first)));
    BOOST_CHECK(!i.check_overlap(p2.second.as_sphere(), array_gen(p2.first)));
    ...
    BOOST_CHECK(!i.check_overlap(p1.second.as_sphere(), array_gen(p1.first)));
    BOOST_CHECK(i.check_overlap(p2.second.as_sphere(), array_gen(p2.first)));
    BOOST_CHECK(i.check_overlap(p2.second.as_sphere(), array_gen(p1.first, p2.first)));
    BOOST_CHECK(!i.check_overlap(p2.second.as_sphere(), array_gen(p2.first, p3.first)));
    BOOST_CHECK(i.check_overlap(p3.second.as_sphere(), array_gen(p3.first)));
    BOOST_CHECK(!i.check_overlap(p3.second.as_sphere(), array_gen(p2.first, p3.first)));
    BOOST_CHECK(i.check_overlap(p3.second.as_sphere(), array_gen(p1.first, p3.first)));

    BOOST_CHECK(!i.check_overlap(p1));
    BOOST_CHECK(i.check_overlap(p2));
    BOOST_CHECK(i.check_overlap(p2, array_gen(p1.first)));
    BOOST_CHECK(!i.check_overlap(p2, array_gen(p3.first)));
    BOOST_CHECK(i.check_overlap(p3));
    BOOST_CHECK(!i.check_overlap(p3, array_gen(p2.first)));
    BOOST_CHECK(i.check_overlap(p3, array_gen(p1.first)));
}
